...
async function executeWorkflow(options: {
  ...
  const releaseRunLock = await acquireRunLock(options.repoPath);
  try {
  ...
  try {
    ...
    if (commands.length > 0) {
      ...
      const results = await runCommands({
        ...
        cwd: repoPath,
        logFile: logPath,
        allowlist: options.config?.shell_allowlist,
        onLine: (line) => {
...
async function acquireRunLock(repoPath: string, timeoutMs = 30000): Promise<() => Promise<void>> {
  const lockPath = path.join(repoPath, ".orchestrum", "run.lock");
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (!existing) {
      await writeText(lockPath, JSON.stringify({ token, pid: process.pid, acquiredAt: nowIso() }));
      return async () => {
        const current = await readTextIfExists(lockPath);
  ...
  throw new Error(`Workspace lock timeout (${timeoutMs}ms): ${lockPath}`);
}
