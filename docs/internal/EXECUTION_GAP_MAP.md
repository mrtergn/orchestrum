...
| `packages/core` browser smoke | real but brittle | real but brittle | Smoke evidence is real; it is still not a deep QA system. |
...
| Plugins | stale / legacy | real but narrow | Mission and browser lifecycle hooks now fire, but the extension surface is still intentionally small. |
...
| Learnings | stale | real and narrower | Mission runs now append workspace learnings automatically, but the signal is still heuristic. |
| Updates | partial / risky | partial / risky | Not part of the trust-critical spine in this tranche. |
| Telemetry | stale | real and opt-in | Mission and browser runs now emit opt-in telemetry, but the event model is still intentionally lightweight. |
...
1. Browser Smoke is still smoke-level, not release-grade.
2. Browser Smoke is still smoke-level and delivery packet operations still rely on thinner secondary code than the mission spine.
